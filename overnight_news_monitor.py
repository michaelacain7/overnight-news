...
JSON response:"""
    ...
    # Filter to score >= 7 (high-impact only)
    for art in articles:
        if art.get("ai_score", 0) >= 7:
            rated.append(art)

    log.info(f"DeepSeek rated {len(rated)} articles as high-impact (score >= 7)")
    return rated
...
def format_discord_embed(article: dict) -> dict:
    ...
    if score >= 9:
        color = 0xFF0000  # red = highest impact
    elif score >= 8:
        color = 0xFFA500  # orange
    else:
        color = 0xFFD700  # gold for 7
...
def run_with_schedule():
    """Run the monitor once daily at 8:00 AM EST, collecting all overnight news (6PM-8AM)."""
    import schedule

    # Schedule for 8:00 AM EST every day
    schedule.every().day.at("08:00").do(run_pipeline)

    log.info("Scheduler started. Will run daily at 8:00 AM EST.")
    log.info("Collecting overnight news from 6:00 PM - 8:00 AM EST window.")

    # Check if we should run immediately (if it's around 8 AM EST now)
    now_est = datetime.now(EST)
    log.info(f"Current time: {now_est.strftime('%I:%M %p EST')}")
